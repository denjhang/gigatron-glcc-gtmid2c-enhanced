...
public:
    ...
    std::vector<CustomMidiEvent> generateMacroEvents(
        ...
        if (vol_release_sequence.empty()) vol_release_sequence = {0};
        ...
        for (size_t i = 0; i < vol_on_sequence.size(); ++i) {
            ...
            if (i < vol_on_sequence.size()) {
                ...
                macro_event.macro_value = 127 - vol_on_sequence[i]; // 转换为127-音量值
        ...
        for (size_t i = 0; i < vol_release_sequence.size(); ++i) {
            ...
            if (i < vol_release_sequence.size()) {
                ...
                macro_event.macro_type = "vol";
                macro_event.macro_value = 127 - vol_release_sequence[i]; // 转换为127-音量值
    ...
    std::vector<CustomMidiEvent> parse(const std::string& filename, double max_duration_seconds, bool dynamic_allocation = false, bool no_velocity_change = false, IniParser* /*config_parser*/ = nullptr, int /*gigatron_ticks_per_second*/ = 60) override {
                ...
                if (message.isNoteOn()) {
                    ...
                    if (_config_parser && _config_parser->hasInstrument(new_event.program)) {
                        ...
                        std::vector<CustomMidiEvent> macro_events = generateMacroEvents(
                            ...
                            0.0, // 不再需要 gigatron_ticks_per_midi_tick，因为 accuracy 已经以 Gigatron ticks 为单位
                            midi_channel
                        );
                        // 将生成的宏事件添加到主事件列表中
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& event : midi_events) {
        long gigatron_tick = static_cast<long>(std::round(event.timestamp * gigatron_ticks_per_midi_tick));
        gigatron_tick = std::max(1L, gigatron_tick);
        
        // 添加延迟
        long delay = gigatron_tick - last_tick;
        if (delay > 0) {
            // 将大延迟分解为多个D(x)调用，每个D(x)最大127
            while (delay > 127) {
            ...
            if (delay > 0) {
                current_segment.push_back("D(" + std::to_string(delay) + ")");
            }
        }
        last_tick = gigatron_tick;
        ...
        if (event.is_macro_event) {
            ...
            // After processing the macro event, add its explicit delay and advance last_tick
            current_segment.push_back("D(" + std::to_string(event.macro_duration) + ")");
            last_tick += event.macro_duration; // Advance last_tick by the macro's explicit delay
            ...
            if (config_parser && config_parser->hasInstrument(event.program)) {
                ...
                if (!volume_sequence.empty()) {
                    // 使用配置文件中的第一个音量值
                    vol = 127 - volume_sequence[0]; // 转换为127-音量值
                    // 确保音量在有效范围内（64-127）
                    vol = std::max(64, std::min(127, vol));
                } else {
                    // 如果没有音量配置，使用默认逻辑
