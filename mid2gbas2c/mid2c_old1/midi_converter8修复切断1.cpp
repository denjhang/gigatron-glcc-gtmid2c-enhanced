            ...
            for (int i = 0; i < midifile[track].size(); ++i) {
                ...
                if (message.isNoteOn()) {
                    ...
                    if (dynamic_allocation) {
                        ...
                        long earliest_available_tick = -1;
                        int candidate_gigatron_channel = -1;

                        // 1. 尝试找到一个当前未被任何MIDI通道映射的Gigatron通道
                        for (int ch = 1; ch <= 4; ++ch) {
                        ...
                        if (candidate_gigatron_channel != -1) {
                            // 找到了一个未被映射的通道
                            ...
                            _debug_log << "[DYNAMIC] Assigned new Gigatron Channel " << gigatron_channel << " to MIDI Channel " << midi_channel << " (unmapped)" << std::endl;
                        } else {
                            // 所有Gigatron通道都被映射了，需要进行替换
                            // 找到最早可用的Gigatron通道
                            for (int ch = 1; ch <= 4; ++ch) {
                            ...
                            // 如果当前事件的开始时间早于最早可用时间，则使用最早可用的通道
                            if (event.tick >= earliest_available_tick) {
                                ...
                                _midi_channel_to_gigatron_channel_map[midi_channel] = gigatron_channel;
                                _gigatron_channel_last_note_on_tick[gigatron_channel] = event.tick;
                                _debug_log << "[DYNAMIC] Assigned Gigatron Channel " << gigatron_channel << " to new MIDI Channel " << midi_channel << " (reassigned)" << std::endl;
                            } else {
                                // 没有可用的Gigatron通道，且当前事件早于最早可用时间，忽略此Note On事件
