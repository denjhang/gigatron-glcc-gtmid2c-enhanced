            ...
            for (int i = 0; i < midifile[track].size(); ++i) {
                ...
                if (message.isNoteOn()) {
                    ...
                    if (dynamic_allocation) {
                        ...
                        // 尝试分配下一个通道
                        gigatron_channel = _next_gigatron_channel_index;

                        // 更新下一个通道索引 (1 -> 2 -> 3 -> 4 -> 1)
                        _next_gigatron_channel_index++;
                        if (_next_gigatron_channel_index > 4) {
                            _next_gigatron_channel_index = 1;
                        }

                        // 检查当前选中的Gigatron通道是否正在被使用
                        if (_gigatron_channel_available_at_tick[gigatron_channel] > event.tick) {
                            // 通道被占用，执行FIFO挤占
                            // 找到被挤占的MIDI通道
                            ...
                            if (midi_channel_to_evict != -1) {
                                _debug_log << "[DYNAMIC] Evicting MIDI Channel " << midi_channel_to_evict << " from Gigatron Channel " << gigatron_channel << " (FIFO, available at tick: " << _gigatron_channel_available_at_tick[gigatron_channel] << ")" << std::endl;
                            ...
                            _debug_log << "[DYNAMIC] Gigatron Channel " << gigatron_channel << " was busy until " << _gigatron_channel_available_at_tick[gigatron_channel] << ", new Note On at " << event.tick << std::endl;
                        } else {
                            // 通道空闲，检查是否已经有MIDI通道映射到此Gigatron通道
                            // 如果有，则解除旧映射
                            int midi_channel_to_unmap = -1;
                            for (auto const& [m_chan, g_chan] : _midi_channel_to_gigatron_channel_map) {
                                if (g_chan == gigatron_channel) {
                                    midi_channel_to_unmap = m_chan;
                                    break;
                                }
                            }
                            if (midi_channel_to_unmap != -1) {
                                _debug_log << "[DYNAMIC] Unmapping MIDI Channel " << midi_channel_to_unmap << " from Gigatron Channel " << gigatron_channel << " (channel now free)" << std::endl;
                                _midi_channel_to_gigatron_channel_map.erase(midi_channel_to_unmap);
                                _gigatron_channel_last_note_on_tick.erase(gigatron_channel);
                            }
                        }
                        ...
                        _debug_log << "[DYNAMIC] Assigned Gigatron Channel " << gigatron_channel << " to MIDI Channel " << midi_channel << " (sequential allocation)" << std::endl;
                    } else {
                        // 静态分配模式
...
private:
    ...
    int _next_gigatron_channel_index = 1; // 新增：下一个要分配的Gigatron通道 (1-4)
...
public:
    MidiFileParserImpl(IniParser* config_parser_ptr = nullptr) : _config_parser(config_parser_ptr) {
        ...
        _next_gigatron_channel_index = 1; // 初始化为1
    }
...
int main(int argc, char* argv[]) {
    ...
    std::vector<CustomMidiEvent> midi_events = parser.parse(midi_filepath, max_duration_seconds, dynamic_allocation, no_velocity_change, gigatron_ticks_per_second);
    ...
    for (const auto& event : midi_events) {
        ...
        if (event.is_macro_event) {
            ...
            if (event.macro_type == "vol") {
                int vol = 127 - event.macro_value; // 应用用户要求的 127 - 音量值
                // 确保音量在有效范围内（64-127）
                vol = std::max(64, std::min(127, vol));
            ...
            double volume_controller_adjusted = std::max(0.0, 127.0 - event.volume);
            double expression_controller_adjusted = std::max(0.0, 127.0 - event.expression);
            double normalized_volume = (base_velocity / 127.0) * (volume_controller_adjusted / 127.0) * (expression_controller_adjusted / 127.0);
            int gigatron_volume = static_cast<int>(std::round(normalized_volume * 63.0));
            
            // 统一音量计算逻辑：始终应用 127 - 音量值 转换
            int current_gigatron_volume = gigatron_volume; // 原始 0-63 范围的音量
            int simplified_vol = simplify_volume(current_gigatron_volume, default_volume_levels);
            int boosted_vol = apply_volume_boost(simplified_vol, min_volume_boost);
            int vol = 127 - boosted_vol; // 应用用户要求的 127 - 音量值
            // 确保音量在有效范围内（64-127）
            vol = std::max(64, std::min(127, vol));
        ...
        // 更新 last_processed_gigatron_tick 到当前事件的开始时间
        last_processed_gigatron_tick = current_event_gigatron_start_tick;
