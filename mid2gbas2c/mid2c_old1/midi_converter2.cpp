    ...
    std::vector<CustomMidiEvent> generateMacroEvents(
        ...
        for (size_t i = 0; i < vol_on_sequence.size(); ++i) {
            ...
            if (i < vol_on_sequence.size()) {
                ...
                macro_event.macro_value = vol_on_sequence[i] + 63; // 增加偏移量63，使音量范围变为64-127
        ...
        for (size_t i = 0; i < vol_release_sequence.size(); ++i) {
            ...
            if (i < vol_release_sequence.size()) {
                ...
                macro_event.macro_type = "vol";
                macro_event.macro_value = vol_release_sequence[i] + 63; // 增加偏移量63，使音量范围变为64-127
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& event : midi_events) {
        long gigatron_tick = static_cast<long>(event.timestamp * gigatron_ticks_per_midi_tick);
        ...
        if (event.is_macro_event) {
            ...
            if (event.macro_type == "vol") {
                ...
                if (channel_states[channel].active && channel_states[channel].vol != vol) {
                    // 添加延迟 D(accuracy) 在音量变化之前
                    int accuracy = event.macro_duration;
                    if (accuracy > 0) {
                        current_segment.push_back("D(" + std::to_string(accuracy) + ")");
                    }
            ...
            if (config_parser && config_parser->hasInstrument(event.program)) {
                ...
                if (!volume_sequence.empty()) {
                    // 使用配置文件中的第一个音量值
                    vol = volume_sequence[0] + 63; // 增加偏移量63
                    // 确保音量在有效范围内（64-127）
                    vol = std::max(64, std::min(127, vol));
                } else {
                    // 如果没有音量配置，使用默认逻辑
