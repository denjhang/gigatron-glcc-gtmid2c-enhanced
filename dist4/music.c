...
{
  gotoxy(1,1);
  cprintf("Gigatron dev7 MIDI Player");
  gotoxy(1,2);
...
void header_time(void)
{
  ...
  if (lastclk != clk) {
    gotoxy(1,4);
    //cprintf("Time: %03ds,Tick:%03d", clk,_clock());
    cprintf("Time: %03ds,", clk);
    lastclk = clk;
  }
    gotoxy(12,4);
    //cprintf("Tick:%03d",_clock());
    cprintf("Wave:%01d,%01d,%01d,%01d",(unsigned char)channel1.wavX,(unsigned char)channel2.wavX,(unsigned char)channel3.wavX,(unsigned char)channel4.wavX);
}
...
void main()
{
  ...
  //sound_sine_waveform(1);
  //sound_sine_waveform(3);
  //sound_sine_waveform(4);
  ...
  gotoxy(1,14);
  cprintf("gtmid2c driver by lb3361\nEnhanced Macro by Denjhang");
