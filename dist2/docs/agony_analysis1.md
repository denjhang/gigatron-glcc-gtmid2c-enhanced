...
*   当 `N(c,n)` 事件发生时，`_midi.p` 会指向音符值 `n`。`code_midi_note()` 会读取 `n`，并根据 `notesTable` 查找对应的频率，然后写入频率寄存器。
*   当 `M(c,n,v)` 事件发生时，`_midi.p` 会先指向音符值 `n`，然后指向音量/波形值 `v`。`code_midi_note()` 会先读取 `n` 设置频率，然后读取 `v` 设置音量和波形。
